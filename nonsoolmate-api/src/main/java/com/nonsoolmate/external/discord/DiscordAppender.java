package com.nonsoolmate.external.discord;

import static com.nonsoolmate.external.discord.exception.ErrorLogAppenderExceptionType.*;
...
import com.nonsoolmate.external.discord.exception.ErrorLogAppenderException;
import com.nonsoolmate.external.discord.model.EmbedObject;
import com.nonsoolmate.global.util.MDCUtil;
import com.nonsoolmate.global.util.StringUtil;
...
public class DiscordAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
	...
	protected void append(ILoggingEvent eventObject) {
		...
		discordWebhook.addEmbed(new EmbedObject().setTitle("[" + level + " - 문제 간략 내용]")
			.setColor(messageColor)
			.setDescription(exceptionBrief)
			.addField("[" + "Exception Level" + "]", StringEscapeUtils.escapeJson(level), true)
			.addField("[문제 발생 시각]", LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
				false)
			.addField("[" + MDCUtil.REQUEST_URI_MDC + "]",
				StringEscapeUtils.escapeJson(mdcPropertyMap.get(MDCUtil.REQUEST_URI_MDC)), false)
			.addField("[" + MDCUtil.USER_IP_MDC + "]",
				StringEscapeUtils.escapeJson(mdcPropertyMap.get(MDCUtil.USER_IP_MDC)), false)
			.addField("[" + MDCUtil.HEADER_MAP_MDC + "]",
				StringEscapeUtils.escapeJson(mdcPropertyMap.get(MDCUtil.HEADER_MAP_MDC).replaceAll("[\\{\\{\\}]", "")),
				true)
			.addField("[" + MDCUtil.USER_REQUEST_COOKIES + "]", StringEscapeUtils.escapeJson(
				mdcPropertyMap.get(MDCUtil.USER_REQUEST_COOKIES).replaceAll("[\\{\\{\\}]", "")), false)
			.addField("[" + MDCUtil.PARAMETER_MAP_MDC + "]", StringEscapeUtils.escapeJson(
				mdcPropertyMap.get(MDCUtil.PARAMETER_MAP_MDC).replaceAll("[\\{\\{\\}]", "")), false)
		...
		if (throwable != null) {
			...
			discordWebhook.addEmbed(new EmbedObject().setTitle("[Exception 상세 내용]")
				.setColor(messageColor)
