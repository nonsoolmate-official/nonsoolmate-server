...
public record GetCouponsResponseDTO(
    @Schema(description = "쿠폰 조회 DTO", example = "") List<GetCouponResponseDTO> coupons) {
  public static GetCouponsResponseDTO of(List<CouponResponseDTO> coupons) {

    List<GetCouponResponseDTO> responseDTOs =
        coupons.stream().map(GetCouponResponseDTO::of).toList();
